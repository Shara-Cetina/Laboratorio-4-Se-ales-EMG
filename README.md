...
### 1- Identificar el mecanismo fisiológico mediante el cual parámetros como la frecuencia media y frecuencia mediana experimentan cambios a medida que el músculo tiende a fatigarse.
<p align="justify">
El desplazamiento de la frecuencia media (MNF) y mediana (MDF) hacia rangos inferiores durante la fatiga muscular tiene su origen principal en la disminución de la velocidad de conducción del potencial de acción a través del sarcolema. Este fenómeno ocurre debido a que el esfuerzo sostenido e intenso provoca una transición hacia el metabolismo anaeróbico, lo que deriva en la acumulación de metabolitos como protones ($H^+$) y lactato. Esta acumulación genera una acidosis intracelular que reduce el pH del músculo, alterando la excitabilidad de la membrana y prolongando la duración de los potenciales de acción individuales. Al volverse estos potenciales más "anchos" en el dominio del tiempo, su representación en el dominio de la frecuencia se concentra en los componentes más bajos del espectro. Adicionalmente, el sistema nervioso central tiende a sincronizar el disparo de las unidades motoras para intentar compensar la pérdida de fuerza, lo que refuerza la presencia de bajas frecuencias y aumenta la amplitud de la señal EMG.
 
### 2- Determinar el alcance y las posibles limitaciones de emplear parámetros del dominio frecuencial en contextos como fisiología del deporte. 
<p align="justify">
El análisis de parámetros frecuenciales en el contexto deportivo ofrece un alcance significativo al permitir una evaluación cuantitativa, objetiva y no invasiva del estado neuromuscular, facilitando la detección precoz de la fatiga incluso antes de que se manifieste una caída en el rendimiento mecánico o la falla muscular. Esto resulta de gran utilidad para caracterizar la predominancia de tipos de fibras musculares en atletas y para optimizar las cargas de entrenamiento, previniendo lesiones por sobreuso. Sin embargo, su aplicación enfrenta limitaciones críticas, como la alta sensibilidad a los artefactos de movimiento propios de la actividad dinámica, los cuales pueden contaminar el espectro de potencia. Asimismo, factores extrínsecos como la temperatura muscular o la impedancia de la piel y el tejido adiposo —que actúa como un filtro pasa-bajo natural— pueden sesgar los valores de MNF y MDF, exigiendo protocolos de filtrado y preparación de la piel sumamente rigurosos para garantizar la validez del diagnóstico.

## Conclusión 
<p align="justify">
La factibilidad de emplear técnicas espectrales para la detección de la fatiga en escenarios no controlados, como el entrenamiento de atletas de alto rendimiento, representa un desafío tecnológico con un alto potencial clínico. En entornos de campo, a diferencia del laboratorio, la señal electromiográfica se ve severamente afectada por artefactos de movimiento dinámicos, cambios en la impedancia de la piel debido a la sudoración y la falta de una postura isométrica constante. A pesar de estas dificultades, el avance en sensores inalámbricos (wearables) y algoritmos de filtrado adaptativo permite que parámetros como la Frecuencia Mediana (MDF) actúen como indicadores objetivos de la carga de trabajo, permitiendo a los entrenadores ajustar las ráfagas de ejercicio antes de que ocurra una falla mecánica o una lesión por sobreuso.

<p align="justify">
Sin embargo, la implementación de estas técnicas en el día a día del atleta requiere una transición del análisis de Fourier convencional hacia métodos más robustos, como la Transformada Wavelet o el análisis tiempo-frecuencia, capaces de procesar señales no estacionarias propias de los movimientos deportivos. La verdadera factibilidad radica en la capacidad de integrar este procesamiento en tiempo real, de modo que la detección del desplazamiento espectral hacia las bajas frecuencias se convierta en una alerta temprana de fatiga metabólica. Aunque factores como la temperatura muscular y la variabilidad del gesto técnico introducen sesgos, el análisis espectral sigue siendo la herramienta más prometedora para objetivar la fatiga muscular en el deporte, siempre que se acompañe de una fase rigurosa de pre-procesamiento de datos.
...
<p align="justify">
Sí, los valores de frecuencia media (MNF) y frecuencia mediana (MDF) presentan una disminución progresiva y una tendencia descendente conforme el músculo alcanza el estado de fatiga. Este fenómeno se atribuye principalmente a la reducción de la velocidad de conducción del potencial de acción muscular (VCPAM). Durante un esfuerzo intenso y sostenido, el metabolismo anaeróbico provoca una acumulación de metabolitos como el lactato y protones ($H^+$), lo que reduce el pH intracelular (acidosis). Esta alteración química afecta la excitabilidad de la membrana y prolonga la duración de los potenciales de acción, haciendo que las ondas eléctricas sean más "lentas" o anchas. En el dominio de la frecuencia, esto se traduce en un desplazamiento de la densidad espectral de potencia hacia las bajas frecuencias, lo cual es detectado matemáticamente por la caída de los indicadores MNF y MDF.
...
<p align="justify">
El uso de la Transformada de Fourier en rehabilitación se justifica por su capacidad para transformar una señal electromiográfica compleja y aparentemente aleatoria en un indicador cuantitativo del estado neuromuscular del paciente. En una terapia, esta herramienta permite a los especialistas evaluar de manera no invasiva la eficiencia de la recuperación, un incremento en las frecuencias altas puede indicar un mejor reclutamiento de unidades motoras rápidas, mientras que el monitoreo de la fatiga espectral ayuda a prescribir cargas de ejercicio seguras y personalizadas. Al proporcionar datos objetivos sobre la resistencia y la fatiga que no son visibles a simple vista, la Transformada de Fourier permite validar la efectividad de un tratamiento de recuperación, asegurando que el músculo está respondiendo correctamente a los estímulos sin riesgo de sobrecarga o lesión.
